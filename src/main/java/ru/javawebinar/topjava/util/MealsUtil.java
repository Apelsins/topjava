...
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
    ...
    public static List<MealWithExceed> createListWithExceed (List<Meal> list) {
        List<MealWithExceed> listWithExceed = new ArrayList<>();
        for (Meal iter : list) {
            listWithExceed.add(
                    MealsUtil.createWithExceed(iter, (Math.round(Math.random()) % 2 == 0) ? true : false));
        }
